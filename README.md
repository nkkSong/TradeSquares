...
> 购物车商店模组（NeoForge 1.21.1）：JSON 配置 + 服务端权威交易 + vanilla 界面。
> 全新实现，参考 SDMShop / ViScriptShop / quest-shop 的思路，不依赖任何重型 UI 库。

## 这是什么 / 为什么做

交易方市诞生于作者自己的服务器需求：现有商店模组更新慢、交互不符合使用习惯，于是从零实现了这个"JSON 配置 + 购物车 + 服务端权威交易"的商店模组。如果你也需要一个轻量、可配置、防刷的 NeoForge 商店方案，它或许同样适合你。
